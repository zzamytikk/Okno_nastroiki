  ...
  */
  C: (b, q) => { //Обработка click 'b = button'
    ...
    if (/zONasO/.test(c = d.attr('class'))) { //Окно открыто `Закрываем`
      ...
      O.O(d, O, O.Fx[d.attr('data-zonas')]); //Отслеживаем изменения размера браузер окна
  ...
  X: (d, o) => { //Закроем окно
    ...
    if(!o && O.Fx[id] && O.Fx[id][0]){//Пользовательская function
      O.Fx[id][0](d);
        ...
        if (W != O.R) { //Изменили размер Браузер окна width
          if (O.R && d[0]) { //Закроем
            ...
            O.T2 = setTimeout(() => {
              ...
              if(f && f[1]){f[1](d)}//функция сработает при изменение браузер экрана
              O.R = W; //Обновим
            }, 500);
          }
